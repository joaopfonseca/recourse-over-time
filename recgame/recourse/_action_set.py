...
from ._actionable_recourse.utils import parse_classifier_args

# todo: add doc string for feasible values
# todo: replace percentiles with scikit-learn API
# todo: get_feasible_values/get_flip_actions should include an option to also include all
#       observed values
...
class ActionSet(object):
    ...
    #### grid generation  ####
    def feasible_grid(
        self, x, return_actions=True, return_percentiles=True, return_compatible=True
    ):
        """
...
#### Constraints ####
SubsetLimitConstraint = namedtuple(
    "SubsetLimitConstraint", ["id", "names", "indices", "lb", "ub"]
)
...
class _ActionElement(object):
    ...
    #### methods ####
    def feasible_values(
        self, x, return_actions=True, return_percentiles=False, drop_suboptimal=True
    ):
        """
