"""
Adapted from:
https://github.com/ustunb/actionable-recourse/blob/master/recourse/flipset.py
"""
...
class Flipset(object):
    ...
    _valid_cost_types = {"total", "local", "max"}
    ...
    def __init__(self, x, action_set, solver="cplex", **kwargs):
        # attach action set
    ...
    def items(self):
        return list(
            map(
                lambda x: dict(zip(self.action_set._names, x["actions"].tolist())),
                self._items,
            )
        )
    ...
    def populate(
        ...
        """
        Generates a list of actions to flip the predicted value of the linear classifier
        from feature vector x.
        ...
        :param time_limit: max # of seconds to spend before stopping the solver at each
                           iteration

        :param node_limit: max # of branch and bound nodes to process before stopping
